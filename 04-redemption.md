...
Unlike other multi-collateral Liquity-based protocols, Moor does not allow redeemers to choose a particular collateral. Instead, redeemers are distributed a pro-rata share of their collateral types. The lowest CR Troves from each collateral type will be chosen during redemption.

For example: Moor consists of 60% collateral units with an oracle market price of $2, and 40% of Liquid Staked collateral units with a market price of $1.8. A USDM holder who redeems 600 USDM will receive 180 collateral units`(60% * 600 / 2)` + 133.33 Liquid Staked collateral units `(40% * 600 / 1.8)` minus the redemption fee.

## Redemption Fee

Similar to Liquity, Moor charges a redemption fee. instead of using a variable baserate, the redemption fee is fixed at 1%.
